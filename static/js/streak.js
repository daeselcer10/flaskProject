...
const streakLastGuess = localStorage.getItem("last_guess_date");
...
if (streakLastGuess){
    const continueLastGuessStreak = new Date(streakLastGuess);
    const alsoTodayToday = new Date();
    if (continueStreak(continueLastGuessStreak, alsoTodayToday)){
        streakCount ++;
    localStorage.setItem("myStreak", streakCount);
    ...
    document.getElementById("streak").innerHTML = streakCount;
        ...
        console.log("Continue streak funct returned false; either time to break the streak, or it's the same day")
    }
}
